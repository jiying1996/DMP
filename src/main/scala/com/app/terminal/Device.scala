...
object Device {
  def main(args: Array[String]): Unit = {
    ...
    df.registerTempTable("t_device")

    val result = sqlContext.sql(
      """
        |select 
        |case devicetype when 1 then "手机" when 2 then "平板" else "其他" end as devicename,
        ...
        |from t_device group by devicetype
      """.stripMargin)
